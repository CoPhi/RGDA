...
if ($fx eq 'form')  then
	if($px eq 'ANY') then  
		$node/../w[ft:query(@normalizedform,$tx)]
	else
...
return 
	<tr class="bk-{$index mod 2}"><p>
		<td class="perica"><b><a href="" title="comment"> {$couple/field[ft:query(@name,'info_ar')]/text()} </a></b>{$perica}</td>
		<td class="pericg"><b><a href="" title="comment"> {$couple/field[ft:query(@name,'info_gr')]/text()} </a></b>{$pericg}</td>
	</p></tr>
}</table>